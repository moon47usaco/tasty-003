...
class TradingControlApp(ctk.CTk):
    ...
    def __init__(self, slider_specs: Optional[Dict[str, SliderSpec]] = None) -> None:
        ...
        self.quantity_var = ctk.DoubleVar(
            value=_clip(
        ...
        self.initial_stop_var = ctk.DoubleVar(
            value=_clip(
        ...
        self.first_target_var = ctk.DoubleVar(
            value=_clip(
        ...
        # NEW: Strike value
        self.strike_text = ctk.StringVar(value="")  # leave empty by default
    ...
    def _build_quantity_section(self) -> None:
        self.quantity_frame, self.quantity_value_lbl = self._section_with_slider(
            row=1, title="Quantity", var=self.quantity_var, spec=self.slider_specs["quantity"]
        )

    def _build_initial_stop_section(self) -> None:
        self.initial_stop_frame, self.initial_stop_value_lbl = self._section_with_slider(
            row=2, title="Init. Stop", var=self.initial_stop_var, spec=self.slider_specs["initial_stop"]
        )

    def _build_first_target_section(self) -> None:
        self.first_target_frame, self.first_target_value_lbl = self._section_with_slider(
            row=3, title="1st Target", var=self.first_target_var, spec=self.slider_specs["first_target"]
        )
    ...
    def _build_footer(self) -> None:
        ...
        # Make 3 columns: [Exit][stretch][Strike + Entry pinned right]
        ...
        # ---- NEW: Strike label + input (bottom-right) ----
        strike_label = ctk.CTkLabel(footer, text="Strike", text_color=TEXT_PRIMARY, font=FONT_BASE)
        strike_label.grid(row=0, column=1, sticky="e", padx=(8, 6))

        self.strike_entry = ctk.CTkEntry(
            footer,
            width=120,
            height=40,
            fg_color=FIELD_BG,
            ...
            border_color=FIELD_BORDER,
            font=FONT_BASE,
            textvariable=self.strike_text,
            placeholder_text="",
        )
        self.strike_entry.grid(row=0, column=2, sticky="e")

    # --------------- Helpers ------------------
    def _section_with_slider(
        self, row: int, title: str, var: ctk.DoubleVar, spec: SliderSpec
    ):
        frame = ctk.CTkFrame(self, fg_color="transparent")
        ...
        # hook after construction to capture label
    ...
    def on_buy(self) -> None:
        ...
        payload = {
            ...
            # optional: include strike if provided (string passthrough)
            "strike": self.get_strike() or None,
        }
        print(payload)
    ...
    # --------------- Public ----------------
    def get_strike(self) -> str:
        return self.strike_text.get().strip()
